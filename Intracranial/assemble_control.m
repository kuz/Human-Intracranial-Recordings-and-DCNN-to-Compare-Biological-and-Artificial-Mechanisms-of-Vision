...
for subject = subjects'
    ...
    % We extract -1700ms to -200ms to take as control signal (no stimulus presented)
    % time coordinates (-870 to -102 tp) 
    s.data = zeros(length(stimseq), length(active_coords.probe_ids), 768);
    for sid = 1:length(stimseq)
        s.data(sid, :, :) = m_data(m_data_ids, (pictimes(sid) - 870):(pictimes(sid) - 103));
    end
    ...
    % store the data
    save(['../../Data/Intracranial/Processed/LFPcontrol/' subject '.mat'], 's');
