...
for fid = 1:length(listing)
    ...
    for i = 1:size(areas, 1)
        ...
        % add area if not seen before
        if ~isKey(area_id_map, areas{i, talareich_level})
            area_id_map(areas{i, talareich_level}) = length(area_id_map) + 1;
            area_id_map_reverse{length(area_id_map)} = areas{i, talareich_level};
        end
    ...
    % clear workspace
    clearvars -except listing talareich_level featureset area_id_map area_id_map_reverse stats
