...
# parameters
print 'Mapping subject %d represented with "%s" to pixels' % (sid, featureset)
...
print 'Loading pixel activations...'
activations = {}
for layer in layers:
...
start = time.time()
results = Parallel(n_jobs=ncores, backend="multiprocessing")(delayed(predict_from_layer)(subject['name'], layer, pid,
...
np.savetxt('../../Data/Intracranial/Probe_to_Layer_Maps/pixels_%s/%s.txt' % (featureset, subject['name']), output, fmt='%.4f')
